...
public static class Validate
{
    ...
    private static readonly Regex ValidHexColor = new Regex("^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$", RegexOptions.Compiled);

    public static void Hex([NotNull] string hex, [NotNull] string parameterName, string? field = null)
    {
        if (string.IsNullOrWhiteSpace(hex))
        {
            throw new ArgumentException($"Hex can't be an empty or whitespace string", parameterName + (field == null ? string.Empty : "." + field));
        }
        //faccio in modo di far passare le email come username
        if (!ValidHexColor.IsMatch(hex))
        {
            throw new ArgumentException("Hex contains invalid characters", parameterName + (field == null ? string.Empty : "." + field));
        }
    }
